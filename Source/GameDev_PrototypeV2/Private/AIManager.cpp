...
void UAIManager::StartLogic(const TArray<AActor*>& Enemies)
{
	...
	for (const AActor* cEnemy : Enemies)
	{
		...
		for (FTileData& cTile : tiles)
		{
			...
			if (!cTile.bIsWalkable || cTile.IsOccupied() || tileOccupated.Contains(cTile.GridTileData.Index))
				continue;
		...
		//if (rBestTile == Gamemode.GetData(cEnemy)
		if (true)
		{
			if (unitToAttack)
			{
